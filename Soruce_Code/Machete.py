...
layout = [[sg.Text('Please select your document and output folder...')],    
	...
	[sg.Text('T. Wallace, v1.1.', size=(60, 1), justification='right')]]    
...
if event=='Generate' and source_filename!='':
	...
	splitlist=[]
	for string in text:
		splitstring = re.split(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s', string)
	...
	with open(statspath, 'w') as f:
		...
		f.write('\n')
		f.write('---\n')
		f.write('Machete version 1.1. Written by Tom Wallace - tomwallace1990@gmail.com')
	f.close()
