...
#Installing Machete
Simply download Machete.exe to anywhere on your computer and double click on it. A black window will appear, which you can ignore, and after a few seconds the main window should appear.
...
#Encoding
Machete uses Microsoft 1252 encoding which should work with all normal files produced on Windows. If you enter text with a different encoding special characters may not display correctly.

#.docx vs .txt
The same text may result in slightly different output depending on if .docx or .txt is used. This issue stems from the way .txt handles new lines. docx is slightly more accurate but the differences are minor.
